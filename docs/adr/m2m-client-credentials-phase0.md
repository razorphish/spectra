...
| OpenAPI | **`GET /openapi.json`** — public limited (merged + filtered). **`GET /integration/openapi.json`** — full spec; **`admin-ui-api`** proxies to **`AVIATE_API_OPENAPI_URL`** with header **`X-Spectra-Integration-Key`** = **`SPECTRA_INTEGRATION_OPENAPI_KEY`** (server env). |
