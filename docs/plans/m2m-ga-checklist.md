...
1. **Phase 1 prod gate** — [m2m-client-credentials-edge-auth.md §GA ordering](m2m-client-credentials-edge-auth.md#ga-ordering-non-negotiable):
   ...
   - [ ] **Staff export / UI:** Admin **Integrations** export and on-screen audit fields expose only plan-allowed columns (no raw access tokens, no `client_secret`); `platform:integrations:export` limited to trusted roles ([Staff visibility](m2m-client-credentials-edge-auth.md#staff-visibility-admin-ui)).
