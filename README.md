...
**Layout:** `apps/spectra-ui`, `apps/sandbox-ui`, `apps/admin-ui`, `apps/local-edge` (local dev helper on **:3000**; not deployed — production public API is **AWS HTTP API Gateway** via Terraform [`api_http`](terraform/modules/api_http)), `apps/platform`, `apps/services/{aviate-api,admin-ui-api,seq-api,quantum-api,corridor-api,ml-camp}`, `packages/{auth,database,logger,openapi,upload,shared-ui}`.
...
**Focused stacks (`focused` group):** **`🛠 Launch Admin stack`** / **`🐛 Debug Admin stack`** (admin API + admin UI), **`🛠 Launch Sandbox stack`** / **`🐛 Debug Sandbox stack`** (aviate + sandbox UI), **`🛠 Launch Platform only`** / **`🐛 Debug Platform only`** (aviate + Expo native). **`🌐 Debug Expo Web Only`** remains the Expo **web** entry. **`local-edge`**, **`seq-api`**, **`quantum-api`**, and **`corridor-api`** are not part of those slices; use **`🟢 Full Stack (Local)`** or start services individually. **`spectra-ui`** (**`local-edge`** on **:3000**) is a separate workflow from the Expo-focused compounds.
...
**Angular → API (local dev):** `nx serve` uses `environment.development.ts` for each app — `spectra-ui` calls **`local-edge`** at `http://127.0.0.1:3000` (see [`apps/local-edge`](apps/local-edge)); `sandbox-ui` → **aviate-api** at `3001`, `admin-ui` → **admin-ui-api** at `3002`. Those services enable permissive CORS so browser requests from `:4200`–`:4202` succeed. In AWS, the public edge is **HTTP API Gateway** (Terraform `api_http`), not this Node process. Staff **Auth0** for `admin-ui` is driven by **`apps/admin-ui/.env`** (or root `.env` `ADMIN_UI_*` keys); see [apps/admin-ui/docs/auth0.md](apps/admin-ui/docs/auth0.md).
