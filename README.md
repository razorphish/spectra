...
**Neon:** copy `.env.example` → `.env`, paste the pooled connection string as `DATABASE_URL`. Then `npm run db:verify`, `npm run db:migrate` (applies SQL under `packages/database/drizzle/`). If `db:migrate` complains about the serverless driver, set `DATABASE_DIRECT_URL` to Neon’s non-pooler connection string (see `.env.example`). With the API running (`nx serve aviate-api`), check `GET /v1/platform/stats` for row counts (still unauthenticated — gate behind the authorizer before prod).
