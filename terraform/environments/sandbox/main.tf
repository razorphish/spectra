...
module "public_api" {
  ...
  description          = "Spectra public control plane (${var.environment})"
...
module "staff_api" {
  ...
  description          = "Spectra staff admin plane (${var.environment})"
  path_segment         = "admin"
...
# Surface the bucket / queue identifiers via SSM so the API runtime can pick
# them up without a redeploy.
resource "aws_ssm_parameter" "uploads_bucket_name" {
  name  = "/${var.project_name}/${var.environment}/uploads/bucket_name"
  type  = "String"
...
resource "aws_ssm_parameter" "uploads_queue_url" {
  name  = "/${var.project_name}/${var.environment}/uploads/queue_url"
  type  = "String"
...
######################################################################
# Lambda deploy bucket — used by spectra-deploy.yml (steps 1/4 wiring)
######################################################################
