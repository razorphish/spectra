output "environment" {
  ...
  description = "Stage label for this sandbox run."
}

output "main_env" {
  value       = var.main_env
  description = "Main env family."
}
...
output "public_api_host" {
  value       = local.public_api_host
  description = "DNS host that will front the public API once Route 53 + ACM are wired."
}

output "staff_api_host" {
  value       = local.staff_api_host
  description = "DNS host that will front the staff API."
}
