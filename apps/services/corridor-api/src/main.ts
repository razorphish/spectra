...
for (const [p, o] of [
  ...
  [join(root, 'apps/services/corridor-api', '.env.development'), true],
] as const) {
  if (existsSync(p)) {
...
/** Target implementation: Go (`/v1/corridor/*`). This Express app is a dev stub. */
...
app.get('/', (_req, res) => {
  res.send({ message: 'Spectra corridor-api (Node stub — replace with Go)', segment: SEGMENT });
});
